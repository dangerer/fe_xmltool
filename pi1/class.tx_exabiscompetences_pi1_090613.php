...
<a id="gtnlogoa" target="_blank" href="http://www.gtn-solutions.com"></a>
			...
			if (t3lib_div::_GP("a")=="c"){
				...
				if (t3lib_div::_GP("b")=="descr"){
					...
					if ($this->has_capability("niveaus")){
						$inhalt.='<b>'.$this->pi_getLL("fuer_sprachen").':</b><br /><span class="fldheader">'.$this->pi_getLL("Niveau").':</span><span class="fldinput">'.$this->create_pulldown("tx_exabiscompetences_niveaus","pd_niveau","title",$rs["niveauid"],true,"","uid","","uid",'',$this->pi_getLL("bitte_Auswahl_treffen"),"","").'</span>';
						...
						$inhalt.='<br><span class="fldheader">'.$this->pi_getLL("Skill").':</span><span class="fldinput">'.$this->create_pulldown("tx_exabiscompetences_skills","pd_skill","title".$this->langcode,$rs["skillid"],true,"","uid","","uid",'',$this->pi_getLL("bitte_Auswahl_treffen"),"","").'</span><br />';
					}
					if($GLOBALS["TSFE"]->fe_user->user["uid"]==1){
							$inhalt.='
							<p>
	...
	function makefilter2(){
		...
		$inhalt.=$this->create_pulldown("tx_exabiscompetences_skills","pd_flt2","title",$this->f2,true,"","uid","WHERE sys_language_uid=".$this->languageid,"uid",'onchange="document.formularflt2.submit();"',$ka_wert=$this->pi_getLL("bitte_Auswahl_treffen"),$joint="",$sqlu="");
		$inhalt.='</form></div>';
		return $inhalt;
	}
	...
	function makelist($tablename,$fegruppe){
		...
		//echo $sql;
			...
			if (t3lib_div::_GP("b")=="descr" && ($anz > 30) && ($filter==false)){
				...
				while($rs=mysql_fetch_array($result)){
					...
					$inhalt.='<tr><td class="tx_exacomp_title">'.$rs["title"].'</td><td class="beispiele_icons"><nobr>';
					if (t3lib_div::_GP("b")=="examp"){
		...
		function getSubjectFilerOptions($multiple,$fv=""){
							...
							$sql="SELECT subj.title,subj.uid,st.title as st_title FROM tx_exabiscompetences_subjects subj INNER JOIN tx_exabiscompetences_schooltypes st ON st.uid=subj.stid ".$whre." ORDER BY st_title,subj.title";
							$result=mysql_query($sql);
					 		$hdr="initialisiert";
					 		while ($rs=mysql_fetch_assoc($result)){
					 			if ($hdr!=$rs["st_title"]){
